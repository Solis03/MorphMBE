"""Headless end-to-end MorphMBE M22 prediction from one RHEED video."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Predict Sq, FSMI and an AFM height map from RHEED video."
    )
...
def main() -> None:
    ...
    selection = analyze_replay(
        source,
        deep_visibility_ranker_path=(repository / config["deep_visibility_ranker"]),
        ...
        foundation_cache_dir=(repository / config["foundation_cache_dir"]),
        ...
        event_policy=str(config.get("replay_event_policy", "best_visible_cycle")),
    ...
    prediction = predictor.predict(
        ...
        seed=(
            int(str(args.sample_id).lstrip("Nn")) * 1_000_003 + event.frame_index * 97
        ),
    )
    np.savez_compressed(
    ...
    payload = {
        ...
        "physics_feature_roi_not_generator_input": asdict(selection.physics_roi.rect),
        "internal_tracking_roi_not_model_input": asdict(selection.tracking_roi.rect),
