...
def extract(
    values: torch.Tensor, timesteps: torch.Tensor, target_shape: tuple[int, ...]
) -> torch.Tensor:
...
class GaussianDiffusionV2:
    def __init__(
        ...
        betas = (
            ...
            else linear_beta_schedule(self.timesteps)
        )
    ...
    def predict_epsilon(
        self, x_t: torch.Tensor, timesteps: torch.Tensor, model_output: torch.Tensor
    ) -> torch.Tensor:
        if self.prediction_target == "epsilon":
            return model_output
    ...
    def training_loss(
        ...
        timesteps = torch.randint(
            0, self.timesteps, (batch,), device=x_start.device, dtype=torch.long
        )
        noise = torch.randn_like(x_start)
        ...
        if cond_dropout > 0:
            keep = (
                (torch.rand(batch, device=x_start.device) >= cond_dropout)
                .float()
                .view(batch, 1)
            )
    ...
    def sample_ddim(
        ...
        indices = np.linspace(
            self.timesteps - 1, 0, int(steps), dtype=np.int64
        ).tolist()
        for step_index, timestep_value in enumerate(indices):
            t = torch.full(
                (shape[0],), int(timestep_value), device=device, dtype=torch.long
            )
