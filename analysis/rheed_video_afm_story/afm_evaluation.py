...
from .afm_descriptors import describe_map, descriptor_distance, gradients
...
def reconstruction_metrics(
    true_unit: np.ndarray, pred_unit: np.ndarray, q_true_nm: float
) -> dict[str, float]:
    ...
    data_range = float(
        max(
            np.percentile(t, 99) - np.percentile(t, 1),
            np.percentile(p, 99) - np.percentile(p, 1),
            1e-6,
        )
    )
    try:
    ...
    metrics = {
        ...
        "physical_ra_error_nm": abs(
            phys_t["true_physical_ra"] - phys_p["pred_physical_ra"]
        ),
        "physical_robust_height_range_error_nm": abs(
            phys_t["true_physical_robust_height_range"]
            - phys_p["pred_physical_robust_height_range"]
        ),
        "physical_histogram_wasserstein_nm": float(
            wasserstein_distance(zt.ravel(), zp.ravel())
        ),
...
def summarize_metrics(scan_metrics: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    for group_key, g in scan_metrics.groupby(keys):
        if not isinstance(group_key, tuple):
            group_key = (group_key,)
        row: dict[str, Any] = dict(zip(keys, group_key, strict=False))
        row["N_scans"] = len(g)
        row["N_groups"] = (
            g["growth_run_id"].nunique() if "growth_run_id" in g.columns else np.nan
        )
...
def group_level_metrics(
    scan_metrics: pd.DataFrame, model_cols: list[str]
) -> pd.DataFrame:
    metric_cols = [
        c
        for c in scan_metrics.columns
    ...
    grouped = (
        scan_metrics.groupby(model_cols + ["growth_run_id"])[metric_cols]
        .median()
        .reset_index()
    )
    return grouped
