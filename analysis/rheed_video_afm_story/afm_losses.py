...
def height_quantile_loss(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    qs = torch.tensor(
        [0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99], device=x.device, dtype=x.dtype
    )
...
def afm_loss(
    pred: torch.Tensor, target: torch.Tensor, preset: str
    ...
    parts = {
        "pixel_l1": float(pixel.detach().cpu()),
        "gradient_l1": float(grad.detach().cpu()),
        "multiscale_l1": float(multi.detach().cpu()),
    }
