...
def _runs(mask: np.ndarray, axis: int) -> tuple[float, float]:
    ...
    return float(longest / max(mask.shape[axis], 1)), float(
        np.mean(spans) if spans else 0.0
    )
...
def _component_features(arr: np.ndarray, threshold: float) -> dict[str, float]:
    ...
    ecc = (
        np.asarray([p.eccentricity for p in props], dtype=float)
        if props
        else np.array([])
    )
    sol = (
        np.asarray([p.solidity for p in props], dtype=float) if props else np.array([])
    )
    circ = []
    for p in props:
    ...
    return {
        ...
        f"component_area_median_p{int(threshold)}": float(np.median(areas) / arr.size)
        if areas.size
        else 0.0,
        f"component_area_q90_p{int(threshold)}": float(
            np.percentile(areas, 90) / arr.size
        )
        if areas.size
        else 0.0,
        f"component_eccentricity_mean_p{int(threshold)}": float(np.mean(ecc))
        if ecc.size
        else 0.0,
        f"component_solidity_mean_p{int(threshold)}": float(np.mean(sol))
        if sol.size
        else 0.0,
        f"component_circularity_mean_p{int(threshold)}": float(np.mean(circ))
        if circ.size
        else 0.0,
        f"round_component_fraction_p{int(threshold)}": float(
            np.mean((circ > 0.45) & (ecc < 0.75))
        )
        if circ.size
        else 0.0,
        f"largest_component_area_fraction_p{int(threshold)}": float(
            areas.max() / arr.size
        )
        if areas.size
        else 0.0,
...
def frame_physics_features(frame: np.ndarray) -> dict[str, float]:
    ...
    for sigma in (1, 2, 4):
        line_responses.append(
            np.abs(filters.sato(arr, sigmas=[sigma], black_ridges=False))
        )
    ...
    neigh = ndimage.convolve(
        skel.astype(int), np.ones((3, 3), dtype=int), mode="constant"
    )
    ...
    border = np.concatenate(
        [arr[:20].ravel(), arr[-20:].ravel(), arr[:, :20].ravel(), arr[:, -20:].ravel()]
    )
    out = {
        ...
        "dominant_orientation": float(
            np.angle(np.sum(np.exp(1j * 2 * orientation) * grad)) / 2
        ),
        ...
        "fft_horizontal_vertical_anisotropy": float(
            (horiz - vert) / (horiz + vert + 1e-9)
        ),
        "row_projection_sharpness": float(
            np.std(row_proj) / (np.mean(row_proj) + 1e-6)
        ),
        "column_projection_sharpness": float(
            np.std(col_proj) / (np.mean(col_proj) + 1e-6)
        ),
        ...
        "spot_peak_width_proxy": float(
            np.sqrt(max(np.sum(bright), 1)) / max(arr.shape)
        ),
        ...
        "diffuse_to_peak_intensity_ratio": float(
            np.percentile(arr, 50) / max(np.percentile(arr, 99), 1e-6)
        ),
        "saturation_fraction": float((frame >= 250).mean()),
        "background_spatial_smoothness": float(
            1.0 / (1.0 + np.std(filters.sobel(arr)))
        ),
        "center_to_border_ratio": float(center.mean() / max(border.mean(), 1e-6)),
    }
    for thr in (90, 95, 97):
        out.update(_component_features(arr, thr))
    out["component_merge_rate_p97_to_p90"] = (
        out["component_count_p90"] - out["component_count_p97"]
    )
    return out
...
def aggregate_temporal(features: list[dict[str, float]]) -> dict[str, float]:
    ...
    out["temporal_stability"] = float(
        1.0 / (1.0 + np.nanmedian([out[k] for k in out if k.endswith("_std")]))
    )
    return out
...
def summarize_categories(row: pd.Series) -> dict[str, float]:
    ...
    return {
        "spot_summary_raw": float(np.nanmean(np.asarray(row[spot_cols], dtype=float)))
        if spot_cols
        else 0.0,
        "streak_summary_raw": float(
            np.nanmean(np.asarray(row[streak_cols], dtype=float))
        )
        if streak_cols
        else 0.0,
        "connection_summary_raw": float(
            np.nanmean(np.asarray(row[conn_cols], dtype=float))
        )
        if conn_cols
        else 0.0,
        "diffuse_summary_raw": float(
            np.nanmean(np.asarray(row[diff_cols], dtype=float))
        )
        if diff_cols
        else 0.0,
    }
...
def extract_rheed_physics_features(
    manifest: pd.DataFrame, config: dict[str, Any]
) -> pd.DataFrame:
    rows = []
    ...
    for _, mrow in manifest.iterrows():
        ...
        row: dict[str, Any] = {
            "sample_id": sid,
            "growth_run_id": str(mrow["growth_run_id"]),
            "video_stage": str(mrow.get("video_stage", "unknown")),
        }
        for variant in config["rheed_clip_variants"]:
            ...
            if variant == "selected_16":
                row["temporal_brightness_drift"] = (
                    float(frames[-1].mean() - frames[0].mean()) / 255.0
                )
