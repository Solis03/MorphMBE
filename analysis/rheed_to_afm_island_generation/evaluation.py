...
def evaluate_island_methods(
    *,
    ...
    train_groups = group_island_feature_table(
        train_rows, resolution=resolution
    )
    held_scans = scan_island_feature_table(
        held_rows, resolution=resolution
    )
    ...
    for method, maps in generated.items():
        features = pd.DataFrame(
            [extract_island_features(array) for array in maps]
        )[ISLAND_FEATURE_COLUMNS].median().to_numpy(float)
