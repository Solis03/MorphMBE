#!/usr/bin/env python3
"""Validate the N6022/N6099-exclusion sensitivity package."""
...
PKG = THIS.parents[1]
HISTORICAL_IDS = [
    "6028", "6029", "6033", "6047", "6048", "6056", "6057", "6062",
...
def main() -> None:
    ...
    add_check(checks, "training_ids_exactly_retained21_plus_6358_6382", normalized_train == TRAIN_IDS, normalized_train)
    add_check(
        checks,
    ...
    add_check(
        checks,
        "N6022_and_N6099_explicitly_excluded",
        split.loc[split["sample_id"].isin(EXCLUDED_IDS), ["sample_id", "role"]].values.tolist()
        == [["6022", "excluded"], ["6099", "excluded"]],
        split.loc[split["sample_id"].isin(EXCLUDED_IDS), ["sample_id", "role"]].values.tolist(),
    )

    baseline = pd.read_csv(PKG / "predictions/reduced_21_baseline/predictions.csv", dtype={"sample_id": str})
    add_check(checks, "reduced21_baseline_has_five_non6324_samples", baseline["sample_id"].tolist() == EXTRA_IDS, baseline["sample_id"].tolist())
    add_check(checks, "reduced21_baseline_training_count", baseline["training_sample_count"].eq(21).all(), baseline["training_sample_count"].tolist())
    predictions = pd.read_csv(PKG / "predictions/retrained_23/predictions.csv", dtype={"sample_id": str})
    add_check(checks, "retrained_prediction_ids_exact", predictions["sample_id"].tolist() == TEST_IDS, predictions["sample_id"].tolist())
    add_check(checks, "retrained_training_count_23", predictions["training_sample_count"].eq(23).all(), predictions["training_sample_count"].tolist())
    ...
    baseline_model_ids_ok = True
    for model_path in sorted((PKG / "models/quantitative_model/reduced_21_baseline").glob("model_*.npz")):
        model = np.load(model_path, allow_pickle=False)
        ids = [str(value) for value in model["training_sample_ids"].tolist()]
        baseline_model_ids_ok &= ids == HISTORICAL_IDS
    add_check(checks, "all_reduced21_model_training_ids_exact", baseline_model_ids_ok, HISTORICAL_IDS)
    ...
    retrieval = pd.read_csv(
        PKG / "predictions/retrained_23/retrieval/retrieval_results.csv",
    ...
    add_check(checks, "retrieval_bank_has_23_groups", retrieval["source_group_count"].eq(23).all(), retrieval["source_group_count"].tolist())
    ...
    add_check(checks, "excluded_samples_never_retrieved_prospectively", not retrieval["source_sample_id"].isin(EXCLUDED_IDS).any(), retrieval["source_sample_id"].tolist())
    ...
    loo_root = PKG / "predictions/leave_one_out_26"
    ...
    add_check(
        checks,
        "loo_prediction_ids_exactly_retained_26",
    ...
    add_check(
        ...
        len(loo) == 26 and loo["sample_id"].is_unique,
        {"rows": len(loo), "unique_ids": int(loo["sample_id"].nunique())},
    )
    add_check(
        checks,
        "loo_all_folds_train_on_25",
        loo["training_sample_count"].eq(25).all()
        and loo_folds["training_sample_count"].eq(25).all()
        and loo_members["training_sample_count"].eq(25).all(),
        {
    ...
    add_check(checks, "loo_held_out_absent_from_every_fold", leakage_ok, "26 fold manifests checked")
    add_check(
        checks,
        "loo_each_fold_uses_exact_other_25_ids",
        fold_membership_ok,
    ...
    hoo_root = PKG / "predictions/held_one_out_afm_26"
    ...
    hoo_bank = pd.read_csv(
        PKG / "models/visual_model/full_labeled_26_afm_bank_manifest.csv",
        dtype={"sample_id": str},
    )
    add_check(
        checks,
        "hoo_afm_prediction_ids_exactly_retained_26",
        hoo_results["sample_id"].tolist() == LOO_IDS,
        hoo_results["sample_id"].tolist(),
    )
    add_check(
        checks,
        "hoo_ground_truth_selection_ids_exactly_retained_26",
        hoo_selections["sample_id"].tolist() == LOO_IDS,
        hoo_selections["sample_id"].tolist(),
    )
    add_check(
        checks,
        "hoo_full_bank_has_26_groups_and_133_maps",
        hoo_bank["sample_id"].nunique() == 26 and len(hoo_bank) == 133,
        {
    ...
    add_check(
        checks,
        "hoo_each_afm_fold_uses_exact_other_25_groups",
        fold_membership_ok
        and hoo_results["fold_source_group_count"].eq(25).all(),
    ...
    add_check(
        ...
        source_exclusion_ok,
        "26 retrieval sources checked",
    )
    add_check(
        checks,
        "hoo_uses_exact_26_fold_loo_rq_predictions",
        prediction_link_ok,
        "tolerance 1e-12 nm",
    )
    add_check(
    ...
    add_check(
        checks,
        "hoo_has_26_per_sample_pair_figures",
        len(hoo_per_sample) == 26,
        len(hoo_per_sample),
    )
    add_check(
        checks,
        "excluded_samples_absent_from_hoo_bank_and_sources",
        not hoo_bank["sample_id"].isin(EXCLUDED_IDS).any()
        and not hoo_results["source_sample_id"].isin(EXCLUDED_IDS).any()
        and not hoo_results["sample_id"].isin(EXCLUDED_IDS).any(),
        {
            "bank_groups": sorted(hoo_bank["sample_id"].unique().tolist()),
            "source_groups": sorted(hoo_results["source_sample_id"].unique().tolist()),
        },
    )

    comparison_dir = PKG / "comparison"
    comparison_files = [
        "aggregate_metric_changes.csv",
        "baseline_prediction_changes.csv",
        "baseline_retrieval_source_changes.csv",
        "full_cohort_metric_changes.csv",
        "held_one_out_afm_common26_changes.csv",
        "leave_one_out_common26_changes.csv",
        "model_parameter_changes.csv",
        "prospective_prediction_changes.csv",
        "prospective_retrieval_source_changes.csv",
        "comparison_provenance.json",
    ]
    add_check(
        checks,
        "all_before_after_comparison_tables_exist",
        all((comparison_dir / name).is_file() for name in comparison_files),
        comparison_files,
    )
    aggregate = pd.read_csv(comparison_dir / "aggregate_metric_changes.csv")
    prospective_change = pd.read_csv(
        comparison_dir / "prospective_prediction_changes.csv",
        dtype={"sample_id": str},
    )
    common_loo_change = pd.read_csv(
        comparison_dir / "leave_one_out_common26_changes.csv",
        dtype={"sample_id": str},
    )
    common_hoo_change = pd.read_csv(
        comparison_dir / "held_one_out_afm_common26_changes.csv",
        dtype={"sample_id": str},
    )
    model_change = pd.read_csv(comparison_dir / "model_parameter_changes.csv")
    full_cohort_change = pd.read_csv(
        comparison_dir / "full_cohort_metric_changes.csv"
    )
    add_check(
        checks,
        "comparison_table_row_counts_exact",
        len(aggregate) == 4
        and len(prospective_change) == 3
        and len(common_loo_change) == 26
        and len(common_hoo_change) == 26
        and len(model_change) == 5
        and len(full_cohort_change) == 2,
        {
            "aggregate": len(aggregate),
            "prospective": len(prospective_change),
            "loo": len(common_loo_change),
            "hoo": len(common_hoo_change),
            "members": len(model_change),
            "full_cohort": len(full_cohort_change),
        },
    )
    aggregate_index = aggregate.set_index("experiment")
    add_check(
        checks,
        "comparison_prospective_mae_matches_raw_tables",
        abs(
            float(aggregate_index.loc["Retrained prospective test", "excluded_MAE_nm"])
            - float(prospective_change["excluded_absolute_error_nm"].mean())
        )
        < 1e-12,
        float(prospective_change["excluded_absolute_error_nm"].mean()),
    )
    add_check(
        checks,
        "comparison_common26_loo_mae_matches_raw_tables",
        abs(
            float(aggregate_index.loc["LOO common retained cohort", "excluded_MAE_nm"])
            - float(common_loo_change["excluded_absolute_error_nm"].mean())
        )
        < 1e-12
        and abs(
            float(aggregate_index.loc["LOO common retained cohort", "original_MAE_nm"])
            - float(common_loo_change["original_absolute_error_nm"].mean())
        )
        < 1e-12,
        {
            "original": float(common_loo_change["original_absolute_error_nm"].mean()),
            "excluded": float(common_loo_change["excluded_absolute_error_nm"].mean()),
        },
    )
    comparison_stems = [
        PKG / "figures/comparison/Figure4_exclusion_impact_summary",
        PKG / "figures/comparison/Figure5_leave_one_out_common26_before_after",
    ]
    ...
    add_check(
        checks,
        "comparison_figures_all_formats_exist",
        all(path.is_file() and path.stat().st_size > 0 for path in comparison_figure_paths),
        [str(path) for path in comparison_figure_paths],
    )
    figure_files = list((PKG / "figures").rglob("*"))
    figure_counts = {
        suffix: sum(path.is_file() and path.suffix == suffix for path in figure_files)
        for suffix in [".png", ".pdf", ".svg"]
    }
    add_check(
        checks,
        "complete_matched_visualization_inventory_plus_comparisons",
        figure_counts == {".png": 72, ".pdf": 18, ".svg": 18},
        figure_counts,
    )
