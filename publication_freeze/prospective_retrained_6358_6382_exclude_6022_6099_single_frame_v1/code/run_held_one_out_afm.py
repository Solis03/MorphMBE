#!/usr/bin/env python3
"""Run 26-fold held-one-out A3 AFM retrieval after excluding 6022/6099."""
...
OUTPUT = PKG / "predictions/held_one_out_afm_26"
BANK_OUTPUT = PKG / "models/visual_model/full_labeled_26_afm_bank_manifest.csv"
...
def build_full_labeled_bank() -> pd.DataFrame:
    ...
    if bank["sample_id"].drop_duplicates().tolist() != SAMPLE_IDS:
        raise RuntimeError(
            "Full labeled AFM bank does not contain the expected ordered 26 sample groups"
        )
...
def calculate_metrics(results: pd.DataFrame) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "evaluation_design": "26-fold held-one-out quantitative fit and AFM retrieval bank; 25 other sample groups per fold",
...
def write_provenance(
    ...
    provenance = {
        "created_at": now(),
        "experiment": "26-fold held-one-out AFM prediction after excluding 6022 and 6099",
        "sample_ids": SAMPLE_IDS,
        "fold_count": 26,
        "quantitative_training_sample_count_per_fold": 25,
        "afm_source_group_count_per_fold": 25,
...
def update_readme() -> None:
    ...
    section = [
        ...
        "## Held-one-out AFM atlas",
        "",
        "The 26-fold AFM experiment uses the existing leave-one-out Rq prediction for each retained sample, excludes that sample's entire AFM group from the A3 retrieval bank, and retrieves morphology from the other 25 retained sample groups. Samples 6022 and 6099 are excluded globally. Its primary atlas is `figures/main/Figure3_held_one_out_afm_prediction_atlas.*`; full results are in `predictions/held_one_out_afm_26/` and `report/held_one_out_afm_summary.md`.",
    ]
...
def main() -> None:
    predictions = pd.read_csv(
        PKG / "predictions/leave_one_out_26/predictions.csv",
        dtype={"sample_id": str},
    )
    if predictions["sample_id"].tolist() != SAMPLE_IDS:
        raise RuntimeError("The 26-sample leave-one-out predictions are missing or reordered")
