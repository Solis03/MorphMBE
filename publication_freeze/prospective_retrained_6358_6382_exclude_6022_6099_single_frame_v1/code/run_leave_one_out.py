#!/usr/bin/env python3
"""Run strict 26-fold leave-one-out after excluding 6022 and 6099."""
...
OUTPUT = PKG / "predictions/leave_one_out_26"
...
def calculate_metrics(predictions: pd.DataFrame) -> dict[str, Any]:
    ...
    metrics: dict[str, Any] = {
        "created_at": now(),
        "evaluation_design": "strict 26-fold leave-one-out; each prediction uses the other 25 labeled samples",
        ...
        "algorithm": {
            ...
            "per_fold_scaling": "StandardScaler fitted on 25 training rows only",
            "per_member_model": "Ridge(alpha=1.0) fitted on 25 training rows only",
            "ensemble_members": 5,
            "aggregation": "median",
        },
    }
...
def make_scatter_figure(predictions: pd.DataFrame, metrics: dict[str, Any]) -> None:
    ...
    categories = [
        ("frozen_historical", "#4C78A8", "o", 48, "Historical cohort (n=21)"),
        (
            "added_extra_five_training",
            "#59A14F",
            "D",
    ...
    ax.set_title(
        "Strict leave-one-out prediction across 26 samples after excluding 6022/6099"
    )
...
def make_diagnostic_figure(predictions: pd.DataFrame, metrics: dict[str, Any]) -> None:
    ...
    for source, color, marker, label, size in [
        ("frozen_historical", "#4C78A8", "o", "Historical (n=21)", 45),
    ...
    fig.suptitle(
        "Strict 26-fold leave-one-out diagnostics (25 training samples per fold)",
        fontsize=13,
        y=1.01,
    )
...
def write_provenance(
    ...
    provenance = {
        "created_at": now(),
        "evaluation_design": "strict leave-one-out across 26 samples after excluding 6022 and 6099",
        "fold_count": len(folds),
        "samples_per_fold_fit": 25,
        ...
        "leakage_checks": {
            ...
            "all_folds_have_25_training_samples": bool(
                folds["training_sample_count"].eq(25).all()
            ),
            "all_members_have_25_training_samples": bool(
                members["training_sample_count"].eq(25).all()
            ),
        },
        "input_files": [
        ...
        json.dumps(
            {
                ...
                "training_sample_count_per_fold": 25,
