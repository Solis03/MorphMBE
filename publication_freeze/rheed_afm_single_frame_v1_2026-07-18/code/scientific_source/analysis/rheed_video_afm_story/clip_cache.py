...
def resize_and_pad(crop: np.ndarray, output_size: int) -> tuple[np.ndarray, float, tuple[int, int, int, int]]:
...
def build_clip_cache(manifest_df: pd.DataFrame, output_root: Path, report_root: Path, output_size: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for row in manifest_df.to_dict("records"):
        ...
        for index in indices:
            ...
            crop_verified = crop_verified and np.array_equal(crop, gray[y : y + h, x : x + w])
        ...
        np.savez_compressed(
            ...
            roi_xywh=np.asarray([row["roi_x"], row["roi_y"], row["roi_width"], row["roi_height"]], dtype=np.int32),
            original_roi_shape=np.asarray([row["roi_height"], row["roi_width"]], dtype=np.int32),
        ...
        quality = compute_quality_metrics(frames, float(row["roi_width"] * row["roi_height"]) / float(row["source_width"] * row["source_height"]))
        quality.update({"sample_id": sample_id, "video_id": row["video_id"], "quality_flags": ";".join(quality_flags(quality))})
        ...
        cache_rows.append(
            {
                ...
                "clip_frame_paths": json.dumps([display_path(frame_path(row["frames_dir"], i)) for i in indices]),
                "clip_frame_indices": json.dumps(indices),
                "keyframe_offset_in_clip": keyframe_offset,
                "roi_area_fraction": float(row["roi_width"] * row["roi_height"]) / float(row["source_width"] * row["source_height"]),
...
def save_clip_preview(frames: np.ndarray, indices: list[int], keyframe_offset: int, row: dict[str, Any], path: Path) -> None:
    tile = 130
    label_h = 20
    cols = 4
    rows = 4
    ...
    for i, frame in enumerate(frames):
        image = Image.fromarray(frame).resize((tile, tile), Image.Resampling.BILINEAR).convert("RGB")
        x = (i % cols) * tile
        ...
        draw.text((x + 4, y + tile + 2), f"{indices[i]}{' key' if i == keyframe_offset else ''}", fill="black")
...
def compute_quality_metrics(frames: np.ndarray, roi_area_fraction: float) -> dict[str, float]:
