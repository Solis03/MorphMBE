#!/usr/bin/env python3
"""Run the 23+N6358+N6382 single-frame retraining and paper-figure workflow."""
...
PKG_REL = Path("publication_freeze/prospective_retrained_6358_6382_single_frame_v1")
PKG = REPO / PKG_REL
...
def copy_frozen_baseline() -> pd.DataFrame:
    source = SOURCE / "predictions/full_cohort_single_frame_v1"
    destination = PKG / "predictions/frozen_23_baseline"
    shutil.copytree(source, destination, dirs_exist_ok=True)
    predictions = pd.read_csv(destination / "predictions.csv", dtype={"sample_id": str})
    if predictions["sample_id"].tolist() != EXTRA_IDS:
        raise RuntimeError(f"Frozen rerun sample IDs do not match expected IDs: {predictions['sample_id'].tolist()}")
    if predictions["sample_id"].isin(IGNORED_IDS).any():
        raise RuntimeError("N6324 unexpectedly appears in the frozen rerun")
    return predictions


def load_embedding_banks() -> tuple[list[str], np.ndarray, dict[str, np.ndarray]]:
    ...
    historical_ids = [str(value) for value in historical["sample_ids"].tolist()]
    ...
    return historical_ids, np.asarray(historical["embeddings"], dtype=float), unseen_map
...
def frozen_reproduction_audit(
    ...
    for member in ensemble["members"]:
        y = target_index.loc[HISTORICAL_IDS, member["target_variant"]].to_numpy(float)
...
def train_and_predict(
    ...
    historical_ids, historical_X, unseen_map = load_embedding_banks()
    ensemble = json.loads(
        (FREEZE / "models/quantitative_model/full_cohort_deployment/ensemble_definition.json").read_text(
            encoding="utf-8"
        )
    )
    reproduction = frozen_reproduction_audit(historical_X, historical_targets, ensemble)
    ...
    output_root = PKG / "predictions/retrained_25"
    ...
    registry = {
        "created_at": now(),
        "model_name": "retrained_25_top5_median_ridge_ensemble",
        ...
        "member_family": "Ridge",
        "ridge_alpha": 1.0,
        "feature_scaling": "StandardScaler fitted on the 25 training rows",
        "aggregation": "median",
        "members": members,
        "frozen_23_reproduction_audit": reproduction,
        "warning": "RETRAINED FULL-COHORT FIT; NOT THE FROZEN STRICT OOF BENCHMARK",
    }
...
def expanded_retrieval_bank(
    ...
    if expanded["sample_id"].astype(str).nunique() != 25:
        raise RuntimeError("Expanded retrieval bank does not contain 25 distinct training groups")
...
def run_retrieval(
    ...
    output = PKG / "predictions/retrained_25/retrieval"
    ...
    for pred in predictions.to_dict("records"):
        ...
        map_path = map_dir / f"{sid}_A3_retrained25_q50_retrieved.npy"
        ...
        rows.append(
            {
                ...
                "rq_condition_source": "retrained_25 predictions.csv",
    ...
    write_json(
        output / "retrieval_run_provenance.json",
        {
            ...
            "training_source_group_count": 25,
        ...
        rows.append(
            {
                ...
                "frozen_23_predicted_rq_nm": old,
                "frozen_23_absolute_error_nm": abs(old - true),
                "retrained_25_predicted_rq_nm": new,
                "retrained_25_absolute_error_nm": abs(new - true),
                "absolute_error_change_nm_retrained_minus_frozen": abs(new - true) - abs(old - true),
            }
        )
    frame = pd.DataFrame(rows)
    y = frame["ground_truth_T4_rq_nm"].to_numpy(float)
    pred = frame["retrained_25_predicted_rq_nm"].to_numpy(float)
    old_pred = frame["frozen_23_predicted_rq_nm"].to_numpy(float)
    metrics = {
...
def make_supplementary_figures(
    ...
    ax.hist(historical["true_T4_rq_nm"], bins=bins, alpha=0.65, label="23 historical", color="#4C78A8")
    ...
    width = 0.25
    ax.bar(x - width, evaluation["frozen_23_predicted_rq_nm"], width, label="Frozen 23")
    ax.bar(x, evaluation["retrained_25_predicted_rq_nm"], width, label="Retrained 25")
    ax.bar(x + width, evaluation["ground_truth_T4_rq_nm"], width, label="Ground truth T4")
    ax.axhline(0, color="black", linewidth=0.7)
    ax.set_xticks(x, evaluation["sample_id"])
    ax.set_ylabel("Rq (nm)")
    ax.set_title("Frozen-model and retrained-model predictions")
    ax.legend()
    save_figure(fig, root, "SuppFigure3_frozen23_vs_retrained25_predictions")

    fig, ax = plt.subplots(figsize=(5.6, 5.3))
    true = evaluation["ground_truth_T4_rq_nm"].to_numpy(float)
    pred = evaluation["retrained_25_predicted_rq_nm"].to_numpy(float)
    ax.scatter(true, pred, s=70, color="#E45756")
    for row in evaluation.itertuples():
        ax.annotate(row.sample_id, (row.ground_truth_T4_rq_nm, row.retrained_25_predicted_rq_nm), xytext=(4, 4), textcoords="offset points")
    ...
    ax.set_title("25-sample full-cohort training fit (not test performance)")
    ax.legend(fontsize=7)
    save_figure(fig, root, "SuppFigure7_retrained25_in_sample_fit")
    ...
    labels = ["Historical train", "Added train", "Predicted", "Ignored"]
    values = [23, 2, 3, 1]
    colors = ["#4C78A8", "#59A14F", "#E45756", "#BAB0AC"]
...
def write_report(
    ...
    baseline = pd.read_csv(PKG / "predictions/frozen_23_baseline/predictions.csv", dtype={"sample_id": str}).set_index("sample_id")
    for sid in TEST_IDS:
        lines.append(
            f"| {sid} | {baseline.loc[sid, 'predicted_rq_nm']:.4f} | "
            f"{pred.loc[sid, 'predicted_rq_nm']:.4f} | "
            f"{eval_index.loc[sid, 'ground_truth_T4_rq_nm']:.4f} | "
            f"{eval_index.loc[sid, 'retrained_25_absolute_error_nm']:.4f} | "
            f"{retrieval_index.loc[sid, 'source_sample_id']} |"
        )
    lines += [
        "",
        f"Retrained three-sample MAE: {metrics['retrained_25']['MAE_nm']:.4f} nm.",
        f"Frozen-baseline three-sample MAE: {metrics['frozen_23_baseline']['MAE_nm']:.4f} nm.",
        "",
    ...
    write_json(
        PKG / "provenance/run_provenance.json",
        {
            ...
            "baseline_rerun_provenance": rel(
                PKG / "predictions/frozen_23_baseline/prediction_run_provenance.json"
            ),
            "primary_figure": rel(
